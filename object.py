...
ap.add_argument("-i", "--image", required=False,
    help="path to the input image (only required if mode is 'image')")
...
if args["mode"] == "video":
    ...
    while True:
        # grab the next frame from the video stream, resize it, and convert it to grayscale
        ret, frame = cap.read()
        if not ret:
            break
        ...
        # check for 'q' key pressed, if pressed break from the loop
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            break
